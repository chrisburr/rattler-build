...
mod source_cache_tests {
    ...
    /// Serves `responses` in order, one per request: each entry is the `Content-Length`
    /// the response advertises paired with the bytes it actually sends. The final entry is
    /// repeated for any further requests. Returns the base URL and a counter of requests.
    ...
    /// A `200 OK` carrying an empty body is a throttled server, not a bad checksum, so the
    /// download is repeated rather than failing the build.
    #[tokio::test]
    ...
    /// A body that stops short of its advertised `Content-Length` is likewise retried.
    #[tokio::test]
    ...
    /// A complete body whose hash disagrees is a stale checksum in the recipe. Repeating
    /// the download would not change the answer, so it fails on the first attempt.
    #[tokio::test]
    ...
    /// A server that never recovers must still fail, rather than retrying forever.
    #[tokio::test]
