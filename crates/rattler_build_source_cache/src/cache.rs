...
/// How many times a URL source is downloaded before the build is failed.
pub(crate) const MAX_DOWNLOAD_ATTEMPTS: u32 = 3;

/// Delay before the second download attempt, doubled for each attempt after that.
...
impl SourceCache {
    ...
    async fn try_url(
        ...
        // Download the file and validate it, repeating the attempt when the server
        // answered successfully but delivered nothing usable.
        let mut attempt = 1;
    ...
    async fn download_url(
        ...
        while let Some(chunk) = stream.next().await {
            // The retry middleware has already handed back a successful response by this
            // point, so a body that dies mid-stream is invisible to it and has to be
            // caught here instead.
        ...
        // A server can promise a length and then stop short. Catching that here keeps a
        // truncated file from reaching checksum validation, where it would be reported as
        // a checksum mismatch and send the user looking for a stale hash in the recipe.
...
/// Whether a checksum failure looks like a download that never actually arrived.
///
/// The HTTP client already retries transport failures, but it only sees the HTTP
/// exchange: a server that answers `200 OK` and then sends nothing looks like a success
/// to it, and the problem first becomes visible here, as a checksum mismatch against a
/// zero-length file. GitLab's `/-/archive/` endpoints generate tarballs on the fly and
/// send the status line before generation begins, so a throttled request produces
/// exactly this.
///
/// A body that arrived in full but hashes differently is far more likely to be a stale
/// checksum in the recipe than a flaky server, so it is reported at once rather than
/// costing repeated downloads of a large archive.
